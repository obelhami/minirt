...
int	setup_3d_world(t_windata *win, t_world_setup *world_setup)
{
	...
	world_setup->hfov = 100;
	fill_vec3(&world_setup->v_camera, 0.0, 0.0, 0.0);
	world_setup->focal_length = 1.0;
	tan_theta = tan(degreed_to_radians(world_setup->hfov) / 2);
	...
	v_upper_left = create_vec3(
			world_setup->v_camera.x - (world_setup->viewport_u.x / 2),
			world_setup->v_camera.y - (world_setup->viewport_v.y / 2),
			world_setup->v_camera.z - world_setup->focal_length);
